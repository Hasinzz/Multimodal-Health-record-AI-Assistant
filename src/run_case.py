...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run local multimodal thesis pipeline."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to X-ray or Brain MRI image.",
    )

    parser.add_argument(
        "--image_modality",
        type=str,
        choices=["xray", "brain_mri"],
        default=None,
        help="Image modality.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to prescription/lab report image, PDF, or TXT file.",
    )

    parser.add_argument(
        "--xray_checkpoint",
        type=str,
        default=str(DEFAULT_XRAY_CHECKPOINT),
        help="Path to X-ray checkpoint.",
    )
    ...
    parser.add_argument(
        ...
        help="Path to Brain MRI checkpoint.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="densenet121",
        help="Backbone name used during training, for example densenet121 or resnet50.",
    )
...
def main():
    ...
    if args.image:
        if not args.image_modality:
            raise ValueError(
                "If --image is provided, you must also provide --image_modality xray or brain_mri."
            )
        ...
        embedding_output_path = (
            case_output_dir / f"{args.case_id}_{args.image_modality}_embedding.npy"
        )
        ...
        save_json(
            model1_output,
            case_output_dir / "model1_output.json",
        )
    ...
    if args.document:
        ...
        save_json(
            model2_output,
            case_output_dir / "model2_output.json",
        )
    ...
    save_json(
        model3_output,
        case_output_dir / "model3_output.json",
    )
